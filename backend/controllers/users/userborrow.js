...
exports.createBorrowRequest = async (req, res) => {
  try {
    ...
    const book = await BookForm.findById(bookId);
    if (!book) {
...
exports.handleBorrowRequest = async (req, res) => {
  ...
  try {
    ...
    const book = await BookForm.findById(borrowRequest.book._id);
    ...
    if (action === 'approve') {
      ...
      const borrowDate = moment().tz('Asia/Kolkata').toDate(); // Current time in Asia/Kolkata
      const dueDate = moment().tz('Asia/Kolkata').add(15, 'minutes').toDate(); // 15 minutes from now in Asia/Kolkata
...
exports.handleReturnRequest = async (req, res) => {
  ...
  try {
    ...
    const book = await BookForm.findById(borrowedBook.book._id);
    if (!book) {
      console.error('Book not found');
      return res.status(404).json({ message: 'Book not found' });
    }

    book.quantity += 1; // Increase the book quantity as it's returned
    await book.save();
    ...
    res.json({ message: 'Book returned successfully, and user history updated.', });
  } catch (error) {
